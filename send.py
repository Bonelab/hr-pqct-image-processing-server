...
class Send:
    ...
    def send(self, base_dir):
        ...
        try:
            ...
            self.logs.log_debug(
                "{} successfully transferred to {}".format(self.image, self.hostname))
            return True
        except Exception:
            self.logs.log_error("Transfer to {} of {} failed".format(self.image, self.hostname))
            return False
