...
# the program
# Created: 2023-05-19
# Dependencies: Anaconda, pytorch, torchvision
import shutil
import threading
...
def main():
    ...
    threading.Thread(target=monitor, args=()).start()  # Passing fn as reference
    # Worker thread
    threading.Thread(target=processing).start()
    # CLI thread
    threading.Thread(target=cli_handle(), args=()).start()
    ...
    while True:
        ...
        match cmd[0]:
            case "jobs":
                ...
                send(jbs, conn, cmd[0])
            case "completed":
                comp = info.get_completed_jobs()
                send(comp, conn, cmd[0])
            case "info":
                inf = info.get_job_com(cmd[1])
                send(inf, conn, cmd[0])
            case "move":
                info.move_queue(cmd[1], cmd[2])
                jbs = info.get_jobs()
                send(jbs, conn, cmd[0])
            case "restart":
                info.restart_job(cmd[1])
                jbs = info.get_jobs()
                send(jbs, conn, cmd[0])
            case "delete":
                info.remove_from_queue(cmd[1])
                jbs = info.get_jobs()
                send(jbs, conn, cmd[0])
            case "quit":
                quit()
...
def monitor():
    ...
    while True:
        if time.time() - last > 3600:                       # Checks every hour to clean up files that are more than a
            info.cleanup(DESTINATION)                       # week old
            last = time.time()
...
def processing():
    while True:
        if info.JOB_QUEUE.not_empty:
            item = info.dequeue()  # First item is gotten from the queue
            info.set_current(item)
            print(f'{item.name} dequeued')  # Log this
            item.process()
            # item.send()
            item.test_send()  # TODO change in final ver to send
            info.set_current(None)
        time.sleep(1)
